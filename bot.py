...
# Fallback se manca battle_system
try:
    from battle_system import TERRENI, DOTTRINE, genera_divisione, calcola_battaglia_tick
except:
    TERRENI = {
        "pianura": {"nome": "Pianura", "atk_mod": 1.0, "def_mod": 1.0, "width": 120, "desc": "Pianura 120 width"},
        "collina": {"nome": "Collina", "atk_mod": 0.85, "def_mod": 1.15, "width": 100, "desc": "Collina 100 width"},
        "foresta": {"nome": "Foresta", "atk_mod": 0.70, "def_mod": 1.30, "width": 80, "desc": "Foresta 80 width"},
        "montagna": {"nome": "Montagna", "atk_mod": 0.55, "def_mod": 1.50, "width": 60, "desc": "Montagna 60 width"},
        "urbano": {"nome": "Urbano", "atk_mod": 0.50, "def_mod": 1.60, "width": 50, "desc": "Urbano 50 width"},
        "deserto": {"nome": "Deserto", "atk_mod": 1.15, "def_mod": 0.80, "width": 140, "desc": "Deserto 140 width"},
        "fortificato": {"nome": "Fortificato", "atk_mod": 0.40, "def_mod": 2.0, "width": 80, "desc": "Fortificato 80 width"},
    }
    DOTTRINE = {
    ...
    def genera_divisione(stats, tipo="fanteria", numero=1):
...
@client.event
async def on_ready():
    print(f"PROELIUM V4 FIX - {client.user} - {len(tree.get_commands())} comandi")
    try:
        if GUILD_ID:
    ...
    if msg.content.strip() == "!force_sync" and is_master(msg.author):
        try:
            if GUILD_ID:
                ...
                tree.copy_global_to(guild=guild)
                synced = await tree.sync(guild=guild)
                await msg.channel.send(f"✅ Force sync {GUILD_ID} -> {len(synced)}: {[c.name for c in synced]} - Master {msg.author.mention}")
            else:
                synced = await tree.sync()
...
async def ping(inter):
    await inter.response.send_message(f"🏓 Sistema V4 fix operativo - Master ID {MASTER_ROLE_ID}", ephemeral=True)
...
async def crea(inter, nome: str, soldi: int, stabilita: int, esercito: int, tecnologia: int, influenza: int, blocco: str, nucleare: bool=False):
    ...
    save_db(db)
    await inter.response.send_message(embed=discord.Embed(title=f"✅ {db[nome]['flag']} {nome} registrata", color=0x00FF00))
...
@tree.command(name="proelium_guerra_dichiara", description="[MASTER] Dichiara guerra")
@app_commands.describe(attaccante="Chi attacca", difensore="Chi difende", motivo="Casus belli", provincia_obiettivo="Obiettivo")
async def guerra_dichiara(inter, attaccante: str, difensore: str, motivo: str, provincia_obiettivo: str="Capitale"):
    ...
    if att not in db or dif not in db:
        await inter.followup.send("Nazione non trovata", ephemeral=True); return
    guerre=load_guerre()
    for g in guerre:
    ...
    embed.set_footer(text=f"Dichiarata da {inter.user.display_name}")
...
async def guerra_lista(inter):
    ...
    for g in attive:
        ...
        txt+=f"**#{g['id']}** {att_f} {g['attaccante']} vs {dif_f} {g['difensore']} | {g['provincia']} | Batt: {g['battaglie']} | {g['inizio']}\n> {g['motivo']}\n\n"
...
@tree.command(name="proelium_guerra_pace", description="[MASTER] Firma pace")
async def guerra_pace(inter, id_guerra: int, condizioni: str="Resa incondizionata"):
    ...
    for g in guerre:
        if g['id']==id_guerra and g['status']=='attiva':
            ...
            embed.add_field(name="📊 Bilancio", value=f"Batt: {g['battaglie']} | Vitt ATT: {g['vittorie_att']} | Vitt DIF: {g['vittorie_dif']}", inline=False)
            await inter.response.send_message(embed=embed)
            return
    await inter.response.send_message(f"Guerra #{id_guerra} non trovata", ephemeral=True)

@tree.command(name="proelium_battaglia", description="[MASTER] Battaglia HOI4 5min ITALIANO")
@app_commands.describe(attaccante="Sigla ATT", difensore="Sigla DIF", terreno="Terreno", dottrina_att="Dottrina ATT", dottrina_dif="Dottrina DIF", provincia="Provincia")
...
async def battaglia(inter, attaccante: str, difensore: str, terreno: str, dottrina_att: str, dottrina_dif: str, provincia: str="Fronte Principale"):
    # FIX CRITICO - defer immediato per evitare "L'applicazione non ha risposto"
    try:
        await inter.response.defer()
    except:
        pass
    ...
    if not is_master(inter.user):
        try:
            await inter.followup.send(f"⛔ Solo Master ID {MASTER_ROLE_ID}", ephemeral=True)
        except:
            pass
        return
    ...
    try:
        ...
        att=attaccante.upper(); dif=difensore.upper()
        if att not in db or dif not in db:
            await inter.followup.send("Nazione non trovata - usa /proelium_lista", ephemeral=True)
            return
        ...
        for tick in range(30):
            ...
            embed.add_field(name=f"🔵 {flag_att} {att} - {len(res['att_engaged'])} Divisioni - Generale {att}", value=txt_att[:1024], inline=False)
            embed.add_field(name=f"🔴 {flag_dif} {dif} - {len(res['def_engaged'])} Divisioni - Generale {dif}", value=txt_dif[:1024], inline=False)
            ...
            if all(d.org<=0 for d in att_divs):
                embed_win=discord.Embed(title=f"🛡️ VITTORIA DIFENSIVA - {dif} {flag_dif}", description=f"{dif} resiste a {provincia}! {att} esaurito", color=0x0000FF)
                embed_win.add_field(name="Esito", value=f"{att} -1 STABILITA", inline=False)
            ...
            if all(d.org<=0 for d in def_divs):
                embed_win=discord.Embed(title=f"🏆 VITTORIA - {att} {flag_att}", description=f"{att} conquista {provincia}! Fronte sfondato", color=0x00FF00)
                embed_win.add_field(name="Esito", value=f"{dif} -1 STABILITA | Freccia {bar} {res['att_percent']}%", inline=False)
                await msg.edit(embed=embed_win, view=None)
        ...
        embed_stallo.add_field(name="Esito", value="Entrambi -1 STABILITA", inline=False)
        ...
        try:
            await inter.followup.send(f"❌ Errore battaglia: {e}\nContatta Master - {e}", ephemeral=True)
            print(f"Errore battaglia: {e}")
        except:
            print(f"Errore battaglia critico: {e}")
...
async def defcon(inter, livello: int=None):
    ...
    data['history'].append(f"{old}->{livello}")
    save_defcon(data)
    await inter.response.send_message(f"DEFCON {old} -> {livello}")

@tree.command(name="proelium_mobilitazione", description="[MASTER] Mobilitazione")
