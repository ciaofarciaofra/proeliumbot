...
# Fallback per sync manuale se slash non appaiono
@client.event
async def on_message(message):
    if message.author.bot:
        return
    if message.content.strip() == "!force_sync":
        # Solo owner o Master puo' forzarlo
        if message.guild and message.guild.owner_id == message.author.id:
            try:
                if GUILD_ID:
                    guild = discord.Object(id=int(GUILD_ID))
                    synced = await tree.sync(guild=guild)
                    await message.channel.send(f"✅ Force sync GUILD {GUILD_ID} -> {len(synced)} comandi: {[c.name for c in synced]}")
                else:
                    synced = await tree.sync()
                    await message.channel.send(f"✅ Force sync GLOBALE -> {len(synced)} comandi (aspetta 1h)")
            except Exception as e:
                await message.channel.send(f"❌ Sync error: {e}")
        else:
            await message.channel.send("Non sei owner, non puoi forzare sync")
...
@client.event
async def on_ready():
    print(f"PROELIUM online come {client.user}")
    try:
        if GUILD_ID:
            guild = discord.Object(id=int(GUILD_ID))
            ...
            await tree.sync()
            print("Sync globale - puo' metterci 1h la prima volta")
    except Exception as e:
